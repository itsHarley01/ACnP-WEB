...
function BookingPage() {
  ...
  async function fetchBookedDates() {
    try {
      ...
      appointments.forEach((app) => {
        const dateKey = new Date(app.preferredDate); // Convert to Date object
        if (app.status === "accepted") {
          dateKey.setDate(dateKey.getDate() - 1); // Subtract 1 day
          const formattedDateKey = dateKey.toISOString().split("T")[0]; // Format to 'yyyy-mm-dd'
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      // Sending the appointment data to EmailJS
      await emailjs.send(
      ...
      // Show success alert and provide redirection
